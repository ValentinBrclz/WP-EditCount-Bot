...
function getTemplatesOnPage(page, callback) {
	client.getArticle(page, function (err, content) {
		...
		var template_regex = new RegExp("{{" + escapeStringRegexp(TEMPLATE) + "(\\|[0-9]+)?(\\|user=.+)?}}", "g");
...
function getUser(template, page) {
	var match = template.match(/\|user=(.+)}}/),
		user;

	if (match === null && page.startsWith("Utilisateur:"))
		user = page.substring(12, page.indexOf('/'));
	else
		user = match[1];
...
function getEditCountForUser(username, callback) {
	...
	client.api.call(params, function (err, info) {
		if (info !== undefined && info.users !== undefined && info.users[0].editcount !== undefined) {
...
function updatePage(page, newcontent, callback) {
	client.logIn(function (err) {
		...
		client.edit(page, newcontent, "Mise à jour du compteur d'édition [[User:Compteur d'édition (bot)|automatique]] (beta)", function (err) {
			if (err) {
				callback(err);
				return;
			}
